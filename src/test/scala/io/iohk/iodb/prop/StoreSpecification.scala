...
class LSMCommands(val maxJournalEntryCount: Int, val keepVersion: Int) extends Commands {
  ...
  class Rollback(version: Version) extends Command {
    ...
    override def nextState(state: State): State = {
      ...
      State(version, state.appendsIndex.view.filterKeys(_ <= version).toMap,
        state.removalsIndex.view.filterKeys(_ <= version).toMap, state.appended.take(ap), state.removed.take(rp))
    }
